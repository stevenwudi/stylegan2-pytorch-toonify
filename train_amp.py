# CUDA_VISIBLE_DEVICES=1,2,3 python -m torch.distributed.launch --nproc_per_node=3 --master_port= train.py
...
def d_r1_loss(real_pred, real_img):
    grad_real, = autograd.grad(outputs=scaler.scale(real_pred.sum()),
                               inputs=real_img, create_graph=True)
    grad_real = [p * inv_scale for p in grad_real]
...
def g_path_regularize(fake_img, latents, mean_path_length, decay=0.01):
    ...
    grad, = autograd.grad(outputs=scaler.scale((fake_img * noise).sum()),
                          inputs=latents, create_graph=True)
    grad_real = [p * inv_scale for p in grad]
    with torch.cuda.amp.autocast():
        grad_norm = 0
        for grad in grad_real:
            grad_norm += grad.pow(2).sum()
        path_lengths = grad_norm.mean()
...
def train(args, loader, generator, discriminator, g_optim, d_optim, g_ema, device):
    ...
    for idx in pbar:
        ...
        if d_regularize:
            ...
            discriminator.zero_grad()
            scaler.scale((args.r1 / 2 * r1_loss * args.d_reg_every + 0 * real_pred[0])).backward()
            scaler.step(d_optim)
            scaler.update()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ckpt", default="../550000.pt", type=str, help="path to the checkpoints to resume training", )
